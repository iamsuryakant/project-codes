...
# a grayscale of the original image by combining the information of k images
# param: filename - the filename of the original image
# param: k  - an integer that must be less than or equal to n. It denotes the number of minimum shares
# param: n  - an integer that denotes the number of images/shares to be created
def image_test(filename, k, n):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            secretid, sharesid = sc.make_random_shares(minimum=k, shares=n, pixel=test1[i][j])
            # convert the large numbers from the resulting shares into values <255
            for shares in sharesid:
    ...
    for i in range(len(all_shares)):
        ...
        nfilename = filename[:-4] + str(i + 1) + ".png"
        print(nfilename)
        # show and save the image
        cv2.imshow(window, all_shares[i])
        cv2.imwrite(nfilename, all_shares[i])
    cv2.waitKey(0)
