...
def restoreImg(pictures, keys):
    for i in range(len(pictures)):
        ...
        img.append(cv2.imread(pictures[i]))
        img[i] = np.asarray(img[i])

    # get dimensions of the restored image and initialize a np array with size of image
    rows = len(img[0])
    cols = len(img[0][0])
    ...
    for i in range(len(pictures)):
        t_mat = np.multiply(img[i][:, :, 0], mat255)
        ac_share.append(np.add(img[i][:, :, 2], t_mat))
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            for k in range(len(pictures)):
                # set up the values of the shares in (k, pub key) form
                shares.append((keys[k], ac_share[k][i][j]))
            # recover the secret value
            restored_image[i][j] = sec.recover_secret(shares)
